...
for (const f of glob.sync('perf/*-baseline.json')) {
  const base = f.replace(/\.json$/,''); const html = base+'.html';
...
<style>body{font:14px sans-serif;margin:16px}table{border-collapse:collapse}td,th{border:1px solid #ddd;padding:6px}th{background:#f5f5f5}</style>
<h1>Perf Summary</h1>
<table>
<thead><tr><th>Page</th><th>Report</th><th>Score</th><th>LCP</th><th>TBT</th><th>TTI</th><th>Transfer</th></tr></thead>
<tbody>${table}</tbody>
</table>`;
fs.writeFileSync('perf/index.html', html); console.log('-> perf/index.html escrito');
